...
impl OclSearchContext {
    pub fn new(rt: OclRuntime) -> Result<Self> {
        ...
        let kernel_func = Kernel::builder()
            ...
            .arg(0.0f32)    // Voxel size
            .arg(&dummy_u64) // Target points
            .arg(&dummy_f32) // Target centroids
            .arg(&dummy_i32) // Target_counts
            .arg(&dummy_i32) // Table remap
            .arg(0) // table size
    ...
    pub fn compute_find_nearest_neighbor(
        ...
        num_source: usize,
        voxel_size: f32,
        target_voxel_ctx: &OclVoxelContext,
        ...
        Self::ensure_buffer(&q, &mut self.buf_indices, num_source, MemFlags::new().read_write())?;
        Self::ensure_buffer(&q, &mut self.buf_dists, num_source, MemFlags::new().read_write())?;

        let d_table_keys = target_voxel_ctx.buf_table_keys.as_ref()
            .context("Target voxel context table keys buffer is None")?;
        let d_table_centroids = target_voxel_ctx.buf_table_centroids.as_ref()
            .context("Target voxel context table centroids buffer is None")?;
        let d_table_counts = target_voxel_ctx.buf_table_counts.as_ref()
            .context("Target voxel context table counts buffer is None")?;
        let d_table_remap = target_voxel_ctx.buf_table_remap.as_ref()
            .context("Target voxel context table remap buffer is None")?;
        ...
        self.kernel_func
            ...
            .context("Failed to set voxel_size arg")?;
        self.kernel_func
            .set_arg(3, d_table_keys)
            .context("Failed to set target_points arg")?;
        self.kernel_func
            .set_arg(4, d_table_centroids)
            .context("Failed to set target_centroids arg")?;
        self.kernel_func
            .set_arg(5, d_table_counts)
            .context("Failed to set target_counts arg")?;
        self.kernel_func
            .set_arg(6, d_table_remap)
            .context("Failed to set table_remap arg")?;
        self.kernel_func
            .set_arg(7, table_size)
            .context("Failed to set table_size arg")?;
        self.kernel_func
            .set_arg(8, d_indices)
        ...
        d_indices.read(&mut indices).enq()
            .context("Failed to read indices")?;
        d_dists.read(&mut dists).enq()
