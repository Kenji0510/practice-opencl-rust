...
use practice_ocl::{
    ...
    operate_pcd_file::{PointXYZ, PointXYZT, load_pcd_xyz, load_pcd_xyzt},
};
...
fn main() -> Result<()> {
    ...
    for i in 0..BENCHMARK_ITERATIONS {
        ...
        let start = Instant::now();
        let (_, _, _, _) = gpu_search
            .compute_find_nearest_neighbor(&d_v_points, v_source_count, VOXEL_SIZE, &gpu_voxel)
        ...
        let (_d_transformed_points, _d_transformed_covs) = gpu_transform
            .apply_transform(&d_v_points, &d_covs, v_source_count, &identity_transform)
